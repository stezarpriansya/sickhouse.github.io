...
class cruang_add extends cruang {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["kode_ruang"] != "") {
				...
				$this->setKey("kode_ruang", $this->kode_ruang->CurrentValue); // Set up key
			} else {
				$this->setKey("kode_ruang", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "ruangview.php")
	...
	function LoadDefaultValues() {
		...
		$this->jenis_ruang->CurrentValue = NULL;
		$this->jenis_ruang->OldValue = $this->jenis_ruang->CurrentValue;
		$this->gedung->CurrentValue = NULL;
		$this->gedung->OldValue = $this->gedung->CurrentValue;
	}
	...
	function LoadOldRecord() {
		...
		$bValidKey = TRUE;
		if (strval($this->getKey("kode_ruang")) <> "")
			$this->kode_ruang->CurrentValue = $this->getKey("kode_ruang"); // kode_ruang
		else
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		// jenis_ruang
		$this->jenis_ruang->SetDbValueDef($rsnew, $this->jenis_ruang->CurrentValue, "", FALSE);

		// gedung
		$this->gedung->SetDbValueDef($rsnew, $this->gedung->CurrentValue, "", FALSE);
		...
		if ($bInsertRow && $this->ValidateKey && strval($rsnew['kode_ruang']) == "") {
...
// Page object
var ruang_add = new ew_Page("ruang_add");
ruang_add.PageID = "add"; // Page ID
var EW_PAGE_ID = ruang_add.PageID; // For backward compatibility

// Form object
var fruangadd = new ew_Form("fruangadd");
...
?>
<form name="fruangadd" id="fruangadd" class="form-horizontal ewForm ewAddForm" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<input type="text" data-field="x_kode_ruang" name="x_kode_ruang" id="x_kode_ruang" size="30" maxlength="6" placeholder="<?php echo ew_HtmlEncode($ruang->kode_ruang->PlaceHolder) ?>" value="<?php echo $ruang->kode_ruang->EditValue ?>"<?php echo $ruang->kode_ruang->EditAttributes() ?>>
</span>
