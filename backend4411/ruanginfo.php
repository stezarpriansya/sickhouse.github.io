...
//
// Table class for ruang
//
class cruang extends cTable {
	var $kode_ruang;
	var $nama_ruang;
	var $jenis_ruang;
	var $gedung;
	...
	function __construct() {
		...
		// kode_ruang
		$this->kode_ruang = new cField('ruang', 'ruang', 'x_kode_ruang', 'kode_ruang', '`kode_ruang`', '`kode_ruang`', 200, -1, FALSE, '`kode_ruang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['kode_ruang'] = &$this->kode_ruang;

		// nama_ruang
		$this->nama_ruang = new cField('ruang', 'ruang', 'x_nama_ruang', 'nama_ruang', '`nama_ruang`', '`nama_ruang`', 200, -1, FALSE, '`nama_ruang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nama_ruang'] = &$this->nama_ruang;

		// jenis_ruang
		$this->jenis_ruang = new cField('ruang', 'ruang', 'x_jenis_ruang', 'jenis_ruang', '`jenis_ruang`', '`jenis_ruang`', 200, -1, FALSE, '`jenis_ruang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['jenis_ruang'] = &$this->jenis_ruang;

		// gedung
		$this->gedung = new cField('ruang', 'ruang', 'x_gedung', 'gedung', '`gedung`', '`gedung`', 200, -1, FALSE, '`gedung`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['gedung'] = &$this->gedung;
	}
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`ruang`";
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('kode_ruang', $rs))
				ew_AddFilter($where, ew_QuotedName('kode_ruang') . '=' . ew_QuotedValue($rs['kode_ruang'], $this->kode_ruang->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`kode_ruang` = '@kode_ruang@'";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		$sKeyFilter = str_replace("@kode_ruang@", ew_AdjustSql($this->kode_ruang->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("ruangview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("ruangview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "ruangadd.php?" . $this->UrlParm($parm);
		else
			return "ruangadd.php";
	}
	...
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("ruangedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("ruangadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("ruangdelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->kode_ruang->CurrentValue)) {
			$sUrl .= "kode_ruang=" . urlencode($this->kode_ruang->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["kode_ruang"]; // kode_ruang
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->kode_ruang->CurrentValue = $key;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->jenis_ruang->Exportable) $Doc->ExportCaption($this->jenis_ruang);
					if ($this->gedung->Exportable) $Doc->ExportCaption($this->gedung);
				} else {
					if ($this->kode_ruang->Exportable) $Doc->ExportCaption($this->kode_ruang);
					if ($this->nama_ruang->Exportable) $Doc->ExportCaption($this->nama_ruang);
					if ($this->jenis_ruang->Exportable) $Doc->ExportCaption($this->jenis_ruang);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->jenis_ruang->Exportable) $Doc->ExportField($this->jenis_ruang);
						if ($this->gedung->Exportable) $Doc->ExportField($this->gedung);
					} else {
						if ($this->kode_ruang->Exportable) $Doc->ExportField($this->kode_ruang);
						if ($this->nama_ruang->Exportable) $Doc->ExportField($this->nama_ruang);
						if ($this->jenis_ruang->Exportable) $Doc->ExportField($this->jenis_ruang);
