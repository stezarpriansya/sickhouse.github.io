...
class cpasien extends cTable {
	...
	function __construct() {
		...
		// kode_pasien
		$this->kode_pasien = new cField('pasien', 'pasien', 'x_kode_pasien', 'kode_pasien', '`kode_pasien`', '`kode_pasien`', 200, -1, FALSE, '`kode_pasien`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['kode_pasien'] = &$this->kode_pasien;

		// nama_pasien
		$this->nama_pasien = new cField('pasien', 'pasien', 'x_nama_pasien', 'nama_pasien', '`nama_pasien`', '`nama_pasien`', 200, -1, FALSE, '`nama_pasien`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['nama_pasien'] = &$this->nama_pasien;

		// alamat_pasien
		$this->alamat_pasien = new cField('pasien', 'pasien', 'x_alamat_pasien', 'alamat_pasien', '`alamat_pasien`', '`alamat_pasien`', 201, -1, FALSE, '`alamat_pasien`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['alamat_pasien'] = &$this->alamat_pasien;

		// jenis_kelamin
		$this->jenis_kelamin = new cField('pasien', 'pasien', 'x_jenis_kelamin', 'jenis_kelamin', '`jenis_kelamin`', '`jenis_kelamin`', 200, -1, FALSE, '`jenis_kelamin`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['jenis_kelamin'] = &$this->jenis_kelamin;

		// tgl_lahir
		$this->tgl_lahir = new cField('pasien', 'pasien', 'x_tgl_lahir', 'tgl_lahir', '`tgl_lahir`', 'DATE_FORMAT(`tgl_lahir`, \'%Y/%m/%d\')', 133, 5, FALSE, '`tgl_lahir`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->tgl_lahir->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['tgl_lahir'] = &$this->tgl_lahir;

		// kota_pasien
		$this->kota_pasien = new cField('pasien', 'pasien', 'x_kota_pasien', 'kota_pasien', '`kota_pasien`', '`kota_pasien`', 201, -1, FALSE, '`kota_pasien`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['kota_pasien'] = &$this->kota_pasien;

		// tgl_datang
		$this->tgl_datang = new cField('pasien', 'pasien', 'x_tgl_datang', 'tgl_datang', '`tgl_datang`', 'DATE_FORMAT(`tgl_datang`, \'%Y/%m/%d\')', 133, 5, FALSE, '`tgl_datang`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->tgl_datang->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['tgl_datang'] = &$this->tgl_datang;

		// tgl_keluar
		$this->tgl_keluar = new cField('pasien', 'pasien', 'x_tgl_keluar', 'tgl_keluar', '`tgl_keluar`', 'DATE_FORMAT(`tgl_keluar`, \'%Y/%m/%d\')', 133, 5, FALSE, '`tgl_keluar`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->tgl_keluar->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['tgl_keluar'] = &$this->tgl_keluar;

		// kode_dokter
		$this->kode_dokter = new cField('pasien', 'pasien', 'x_kode_dokter', 'kode_dokter', '`kode_dokter`', '`kode_dokter`', 200, -1, FALSE, '`kode_dokter`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`pasien`";
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			if (array_key_exists('kode_pasien', $rs))
				ew_AddFilter($where, ew_QuotedName('kode_pasien') . '=' . ew_QuotedValue($rs['kode_pasien'], $this->kode_pasien->FldDataType));
		}
		$filter = $this->CurrentFilter;
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function SqlKeyFilter() {
		return "`kode_pasien` = '@kode_pasien@'";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		$sKeyFilter = str_replace("@kode_pasien@", ew_AdjustSql($this->kode_pasien->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			return $this->KeyUrl("pasienview.php", $this->UrlParm($parm));
		else
			return $this->KeyUrl("pasienview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}

	// Add URL
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "pasienadd.php?" . $this->UrlParm($parm);
		else
			return "pasienadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("pasienedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("pasienadd.php", $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("pasiendelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->kode_pasien->CurrentValue)) {
			$sUrl .= "kode_pasien=" . urlencode($this->kode_pasien->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["kode_pasien"]; // kode_pasien
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->kode_pasien->CurrentValue = $key;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->nama_pasien->Exportable) $Doc->ExportCaption($this->nama_pasien);
					if ($this->alamat_pasien->Exportable) $Doc->ExportCaption($this->alamat_pasien);
					...
					if ($this->kota_pasien->Exportable) $Doc->ExportCaption($this->kota_pasien);
					if ($this->tgl_datang->Exportable) $Doc->ExportCaption($this->tgl_datang);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->nama_pasien->Exportable) $Doc->ExportField($this->nama_pasien);
						if ($this->alamat_pasien->Exportable) $Doc->ExportField($this->alamat_pasien);
						...
						if ($this->kota_pasien->Exportable) $Doc->ExportField($this->kota_pasien);
						if ($this->tgl_datang->Exportable) $Doc->ExportField($this->tgl_datang);
