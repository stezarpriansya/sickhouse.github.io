...
class clogin extends cuser {
	...
	function Page_Main() {
		...
		if (IsLoggingIn()) {
			...
			$this->Username = $_SESSION['username']; // Initialize
			if (@$_POST["username"] <> "") {

				// Setup variables
				$this->Username = ew_RemoveXSS(ew_StripSlashes($_SESSION['username']));
				$sPassword = ew_RemoveXSS(ew_StripSlashes($_SESSION['password']));
