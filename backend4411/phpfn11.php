...
function ew_AutoLoad($class) {
	...
	if (substr($class, 0, 1) == "c") {
		if ($class == "crumahsakitfp_db") {
			$file = "rumahsakitfpdb.php";
		} else {
		$fn = "%cls%info.php";
...
function CurrentProjectID() {
	...
	return "{E4257960-51C0-4B8A-8F41-FDCC3F20971D}";
}
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($ValidateUser) {
						...
						if (is_null($rs->fields('tipe'))) {
							$this->setSessionUserLevelID(0);
						} else {
							$this->setSessionUserLevelID(intval($rs->fields('tipe'))); // Load User Level
						}
						$this->SetUpUserLevel();
