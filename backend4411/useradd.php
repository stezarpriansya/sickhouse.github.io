	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if (strval($this->tipe->CurrentValue) <> "") {
				switch ($this->tipe->CurrentValue) {
					case $this->tipe->FldTagValue(1):
						$this->tipe->ViewValue = $this->tipe->FldTagCaption(1) <> "" ? $this->tipe->FldTagCaption(1) : $this->tipe->CurrentValue;
						break;
					case $this->tipe->FldTagValue(2):
						$this->tipe->ViewValue = $this->tipe->FldTagCaption(2) <> "" ? $this->tipe->FldTagCaption(2) : $this->tipe->CurrentValue;
						break;
					default:
			...
			$arwrk = array();
			$arwrk[] = array($this->tipe->FldTagValue(1), $this->tipe->FldTagCaption(1) <> "" ? $this->tipe->FldTagCaption(1) : $this->tipe->FldTagValue(1));
			$arwrk[] = array($this->tipe->FldTagValue(2), $this->tipe->FldTagCaption(2) <> "" ? $this->tipe->FldTagCaption(2) : $this->tipe->FldTagValue(2));
...
fuseradd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $user->tipe->FldCaption(), $user->tipe->ReqErrMsg)) ?>");
...
<input type="text" data-field="x_username" name="x_username" id="x_username" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($user->username->PlaceHolder) ?>" value="<?php echo $user->username->EditValue ?>"<?php echo $user->username->EditAttributes() ?>>
</span>
...
<input type="text" data-field="x_password" name="x_password" id="x_password" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($user->password->PlaceHolder) ?>" value="<?php echo $user->password->EditValue ?>"<?php echo $user->password->EditAttributes() ?>>
</span>
<?php echo $user->password->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($user->tipe->Visible) { // tipe ?>
	<div id="r_tipe" class="form-group">
		<label id="elh_user_tipe" for="x_tipe" class="col-sm-2 control-label ewLabel"><?php echo $user->tipe->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="col-sm-10"><div<?php echo $user->tipe->CellAttributes() ?>>
<?php if (!$Security->IsAdmin() && $Security->IsLoggedIn()) { // Non system admin ?>
<span id="el_user_tipe">
<p class="form-control-static"><?php echo $user->tipe->EditValue ?></p>
</span>
<?php } else { ?>
<span id="el_user_tipe">
<select data-field="x_tipe" id="x_tipe" name="x_tipe"<?php echo $user->tipe->EditAttributes() ?>>
<?php
if (is_array($user->tipe->EditValue)) {
	$arwrk = $user->tipe->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($user->tipe->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
...
<?php } ?>
<?php echo $user->tipe->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
